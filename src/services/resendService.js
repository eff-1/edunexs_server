...
export const sendWelcomeEmailResend = async (email, name, role) => {
  try {
    ...
    const { data, error } = await resend.emails.send({
      ...
      `,
      text: `Welcome to Edunexs LearnSphere, ${name}! Your account has been verified successfully. Visit ${process.env.CLIENT_URL || 'https://edunexs-client.vercel.app'}/dashboard to get started.`
    })
